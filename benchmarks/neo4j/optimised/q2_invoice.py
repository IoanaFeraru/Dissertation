"""
benchmarks/neo4j/optimised/q2_invoice.py — Neo4j Optimised: Q2
===============================================================
Q2: Fetch a complete invoice with customer info, all line items,
    and full product details.

Optimised schema: same Cypher as naive — the schema additions
(ALSO_BOUGHT, full-text index, composite event index, native cart)
do not affect Q2. No schema effect to measure here; this is engine
effect only. Documented in CHANGES.md.
...
def fetch_invoice_id_pool(driver, pool_size: int) -> list[str]:
    ...
    if not ids:
        raise RuntimeError(
            "No Invoice nodes found. Run the optimised loader before benchmarking."
        )
...
def dry_run(driver, invoice_ids: list[str]):
    ...
    print(f"\n  DRY RUN — Neo4j Optimised Q2 result for invoice {invoice_id}:\n")
    with driver.session() as session:
        result = session.run(Q2_CYPHER, invoice_id=invoice_id)
        rows = result.data()
...
def main():
    parser = argparse.ArgumentParser(description="Neo4j Optimised Q2 benchmark")
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            ...
            "neo4j_optimised_Q2.json",
        ),
    )
    args = parser.parse_args()

    print("\n" + "=" * 55)
    print("  Neo4j Optimised — Q2 Invoice Fetch Benchmark")
    print("=" * 55)
    ...
    try:
        ...
        run_benchmark(
            ...
            label=(
                ...
                "Cypher identical to naive — schema optimisations do not affect Q2. "
                "Measures engine effect only. "
